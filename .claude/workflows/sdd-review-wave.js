export const meta = {
  ...
  description:
    "Review a wave of completed SDD tasks in parallel, refute each finding adversarially, then look for interactions between the tasks",
  whenToUse:
    "When two or more SDD tasks complete and need reviewing before integration. Pass args: [{task, base, head, worktree?, brief?, scope}]",
  phases: [
      ...
      items: {
        ...
        properties: {
          ...
          isDisclosure: {
            type: "boolean",
            description:
              "True if this lets someone read data they should not. Ranked first regardless of severity.",
          },
          provenByTest: {
            type: "boolean",
  ...
  (t) =>
    agent(
      [
        ...
        t.worktree
          ? `The work is in worktree: ${t.worktree}`
...
const interactions = await agent(
  [
    ...
    ...tasks.map(
      (t) =>
        `- ${t.task}: ${t.base}..${t.head}${t.worktree ? ` (worktree ${t.worktree})` : ""}${t.scope ? ` — ${t.scope}` : ""}`,
    ),
    "",
