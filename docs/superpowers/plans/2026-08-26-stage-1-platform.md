...
- [x] ~~Feature parity on CI~~ — **RESTATED 2026-09-20, and met as restated:** CI is green on typecheck, lint, format:check, build, the dev bootstrap and the full test suite, with the tenant-isolation and route-access gates among them. The original wording asked for parity with a predecessor that does not exist: the web client was already inert before Phase E (no credential reached PostgREST, so `get_current_town_id()` could not resolve and every read returned zero rows), which is why `docs/superpowers/specs/2026-08-29-phase-e-web-restoration-design.md:25` states there is no parity baseline. Phase E was a restoration, not a migration. Evidence and the full reasoning: `phase-f-stage-1-gate.md` § 8. A functional end-to-end suite — the thing actually worth having, and NOT the same as parity — is [backlog entry 22](../../backlog.md).
