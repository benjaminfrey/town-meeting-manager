...
supabase
  .from("meeting_attendance")
  .upsert(
    { id: crypto.randomUUID(), meeting_id: meetingId, board_member_id: memberId, status },
...
supabase
  .from("vote_record")
  .upsert(
    { id: crypto.randomUUID(), motion_id: motionId, board_member_id: memberId, vote },
...
supabase
  .from("vote_record")
  .insert({ motion_id: motionId, board_member_id: memberId, vote: "recused" });
```
...
supabase
  .from("meeting")
  .update({ status: "adjourned", ended_at: new Date().toISOString() })
  .eq("id", meetingId);
// Invalidate: queryKeys.meetings.detail(meetingId)
```
