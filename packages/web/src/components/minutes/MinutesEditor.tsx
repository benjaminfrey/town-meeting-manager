...
import { Save, ChevronDown, ChevronRight, Check, Clock, Loader2 } from "lucide-react";
...
export function MinutesEditor({
  ...
  const updateItemField = useCallback(
    (
      ...
      setEditableContent((prev) => {
        ...
        (next.sections[sectionIdx]!.items[itemIdx]! as unknown as Record<string, unknown>)[field] =
          value;
        return next;
      });
      markDirty();
  ...
  const updateSpeakerName = useCallback(
    (sectionIdx: number, itemIdx: number, speakerIdx: number, name: string) => {
      setEditableContent((prev) => {
        const next = structuredClone(prev);
        next.sections[sectionIdx]!.items[itemIdx]!.speakers[speakerIdx]!.name = name;
        return next;
      });
      markDirty();
    },
    ...
    <div className="flex h-full">
      ...
      <div className="flex w-[60%] flex-col border-r border-border">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {editableContent.sections.map((section: MinutesContentSection, sectionIdx: number) => {
                ...
                {!isCollapsed && (
                  <div className="border-t border-border px-4 pb-4 pt-3 space-y-4">
                    {section.items.length === 0 && section.marked_none && (
                      <p className="text-xs italic text-muted-foreground">Marked as none.</p>
                    )}

                    {section.items.map((item: MinutesContentItem, itemIdx: number) => (
                      <div
                        key={itemIdx}
                        className="space-y-2 rounded border border-border bg-background p-3"
                        onClick={() => setSelectedSectionIndex(sectionIdx)}
                      >
                        {/* Item title */}
                        <div>
                          ...
                          <input
                            ...
                            value={item.title}
                            onChange={(e) =>
                              updateItemField(sectionIdx, itemIdx, "title", e.target.value)
                            }
                        ...
                        {(item.timestamp_start || item.timestamp_end) && (
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            <span>
                              {item.timestamp_start ?? "--"} &rarr; {item.timestamp_end ?? "--"}
                            </span>
                          </div>
                        )}
                              ...
                              {item.motions.map((motion, mIdx) => (
                                ...
                                >
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {motion.moved_by && <span>Moved: {motion.moved_by}</span>}
                                    {motion.seconded_by && (
                                      <span>Seconded: {motion.seconded_by}</span>
                                    )}
                                    {motion.vote && (
                              ...
                              {item.speakers.map((speaker, sIdx) => (
                                <div key={sIdx} className="flex items-center gap-2">
                                  <input
                                    ...
                                    value={speaker.name}
                                    onChange={(e) =>
                                      updateSpeakerName(sectionIdx, itemIdx, sIdx, e.target.value)
                                    }
