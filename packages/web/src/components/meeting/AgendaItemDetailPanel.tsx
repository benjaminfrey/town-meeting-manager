...
import { MotionCaptureDialog, type MotionDialogMode } from "./MotionCaptureDialog";
...
export function AgendaItemDetailPanel({
  ...
  const [itemRecusals, setItemRecusals] = useState<Map<string, string>>(new Map());
  ...
  const activeVotingMotionId = item?.motions.find((m) => m.status === "in_vote")?.id ?? null;
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center text-center">
        ...
        <p className="mt-2 max-w-sm text-sm text-muted-foreground">
          Public recording is paused during executive session. No agenda item content or motions are
          displayed. Use the banner above to return to public session.
        </p>
      </div>
    );
  }
  ...
  const isPublicComment =
    item.sectionType === "public_input" || item.sectionType === "public_hearing";
      ...
      {quorumBlocked && (
        <div className="border-b border-red-200 bg-red-50 px-6 py-3 dark:border-red-900 dark:bg-red-950/30">
          <div className="flex items-center gap-2 text-sm font-medium text-red-700 dark:text-red-400">
            ...
            QUORUM ALERT: Recusals drop participating members to {quorumCheck!.eligibleVoters}.
            Quorum requires {quorumCheck!.adjustedQuorum}. This vote cannot proceed.
          </div>
        </div>
      )}
