/**
 * `MeetingRolesEditor`'s cache invalidation — see
...
import { MeetingRolesEditor } from "../MeetingRolesEditor";
...
const stub = installTRPCFetchStub({
  "town.updateMeetingRoles": (input) => input,
});
...
async function save() {
  ...
  const { user } = renderWithProviders(
    <MeetingRolesEditor townId="town-1" initial={initial} onDone={() => {}} />,
    { queryClient },
  );

  await user.click(screen.getByRole("button", { name: /save/i }));
  await waitFor(() => expect(stub.countFor("town.updateMeetingRoles")).toBeGreaterThan(0));

  return { legacyKey, detailKey };
}

describe("MeetingRolesEditor cache invalidation", () => {
