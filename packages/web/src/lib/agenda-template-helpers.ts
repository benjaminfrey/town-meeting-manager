 ...
 */
export function parseSections(jsonText: string | null): AgendaTemplateSection[] {
  if (!jsonText) return [];
  try {
 ...
 */
export function serializeSections(sections: AgendaTemplateSection[]): string {
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (!result.success) {
      throw new Error(`Invalid section at index ${i}: ${result.error.message}`);
    }
